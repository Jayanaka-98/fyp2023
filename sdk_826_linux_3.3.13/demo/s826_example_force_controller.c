...
static double J_n = 0.268/10000;								// Mass : (Motor shaft, 2 x linera bearings, 1 x Long green bearing and shft connector 1 x short green 											//bearing and shft connector)=600g and (whight hammer head)=55g .   Mass : (Motor shaft, 2 x linera bearings, 1 x Long green bearing and shft connector 1 x short green bearing and shft connector)=600g and (whight hammer head)=55g . Changed the hammer head mass to 43 from 57 from elctronic balance, thus the new mass =  0.655 - 55 + 43 =0.643g 
...
static double g_velocity = 400.0;									// velocity filter constent
...
//		I_a_ref = K_p_R*(F_ref_R*0.01591 - rtob_torque_R)/K_tn;
		...
		// Linear Master Motor Force Controller

		I_a_ref_M = K_p*(-rtob_torque_R/0.01591 - rtob_torque)/(K_tn_M);
		I_motor_M = I_a_ref_M + (dob_torque/K_tn_M);

		// Linear Master Motor Position Control
...
//		I_motor_M = I_a_ref_M + (dob_torque/K_tn_M);
		printf("%f,%f,%f\n",I_a_ref_M,rtob_torque,I_motor_M);
		fprintf(fp,"%d,%f,%f,%f,%f\n",timestamp,position_x_R,position_x,rtob_torque_R/0.01591,rtob_torque);
...
//	analog_out_motor = (unsigned int)(32768 + (I_motor/0.6)*32768);			// DAC senstivity 16 bit (2**16)movo zero vlaue 0 -->  2.5V out in dac
	analog_out_motor = (unsigned int)(I_motor*65635);
