...
// motor parameters
float K_fn_m = 24.0;										// Master motor force constent
float M_n_m = 0.643;										// Mass : (Motor shaft, 2 x linera bearings, 1 x Long green bearing and shft connector 1 x short green bearing and shft connector)=600g and (whight hammer head)=55g .
float friction_m = 0.0;

float K_tn_r = 0.135;										// Replica motor force constent
float M_n_r = (0.268/10000)/(0.01591*0.01591);
float J_n_r = 0.268/10000;										// Innertia : (Motor shaft, 2 x linera bearings, 1 x Long green bearing and shft connector 1 x short green bearing and shft connector)=600g and (whight hammer head)=55g .
float friction_r = 0.0;

// Velocity mesurment
float velocity_sum_m = 0.0;
float velocity_m = 0.0;										// motor velocity
float velocity_prev_m = 0.0;
float velocity_prev_r = 0.0;
...
float g_velocity_m = 300.0;									// velocity filter constent
float g_velocity_r = 300.0;									// velocity filter constent
...
//	printf("counts:%d \n",counts_plotform_motor);
	...
	{	
        ...
        x_error_m = ((position_x_r*0.01591)*alpha - position_x_m);
        out_PD_m = K_p_m * x_error_m + K_d_m * (velocity_m - velocity_prev_m);
        x_error_m_prev = x_error_m;

        x_error_r = (position_x_m/0.01591 - position_x_r*alpha);
        out_PD_r = K_p_r * x_error_r + K_d_r * (velocity_r - velocity_prev_r);
        ...
        f_r = (out_PD_r * 0.01591 / (alpha + beta) - f_error/(alpha + beta) * C_f)*(M_n_m*M_n_r/(M_n_r+M_n_m));

        I_motor_m = (f_m + dob_force_m) / K_fn_m;
        I_motor_r = (f_r*0.01591 + dob_torque_r) / K_tn_r;

		MotorOutDAC(board);	// the motor input current
		
		fprintf(fp,"%f,%d,%d\n",function_exicution_time,counts_plotform_motor,counts_rotory_motor);
	
		velocity_prev_m = velocity_m;
...
static int MotorOutDAC(uint board)
{	
	if (I_motor_m < -0.59)        { I_motor_m = -0.59;
	}else if(I_motor_m > 0.59)	{ I_motor_m = 0.59;    }

    if (I_motor_r < -1.0)        { I_motor_r = -1.0;
...
//	analog_out_motor = (unsigned int)(32768 + (I_motor/1.5)*32768);			// DAC senstivity 16 bit (2**16)movo zero vlaue 0 -->  2.5V out in dac
	analog_out_motor_m = (unsigned int)((65535*0.5) + ((I_motor_m*65535)/(0.6*2)));					// DAC senstivity 16 bit (2**16)movo zero vlaue 0 -->  0V out in dac
...
static float VelocityCalculation()
{
	...
	velocity_m = g_velocity_m * (position_x_m - velocity_sum_m);

    position_x_r = steps_r*PI/10000;
...
static int DoB()
{
	...
	dob_filter_input_m = K_fn_m * I_motor_m + dob_input_m;
    ...
    dob_input_r = velocity_r * g_dis * J_n_r;
