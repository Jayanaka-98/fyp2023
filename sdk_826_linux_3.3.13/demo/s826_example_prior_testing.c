...
uint aout = 2;        								// aout= analogue output channel no.2
uint setpoint;
uint range;
...
// motor parameters
static double K_fn = 24.0;								// motor force constent
...
int const g_dis = 100.0;							//Consider g_rec = g_dis
...
// PID
static double K_p = 1400.0;
static double K_I = 0.0;//0.0;
static double K_d = 100.0;
...
static int ControlLoop(uint board)
{
	...
	X826( S826_CounterModeWrite(board, 4, S826_CM_K_QUADX4));  			// Configure counter0 as incremental encoder interface. quadrature

	X826( S826_CounterStateWrite(board, 4, 1));  						// Start tracking encoder position.	
	
	I_motor = 0.0;	
	position_x = 0.0; 													//stop the mortor input current
	
	MotorOutDAC(board);  //- remove
	X826( S826_CounterSnapshot(board, 4) );     					    				// Trigger snapshot on counter 4.
	X826( S826_CounterSnapshotRead(board, 4, &counts, &timestamp_start, NULL, 0));      // Read the snapshot: from the counter4
	...
	X826( S826_CounterSnapshot(board, 4) 						);     // Trigger snapshot on counter 0.
	X826( S826_CounterSnapshotRead(board, 4, &counts, &timestamp, NULL, 0) 	);     // Read the snapshot:receive the snapshot info here;no need to wait for snapshot;it's already been captured
...
//	fp = fopen("testdata_V02_3.txt", "a+");
	...
	//K_I = 10; //5.0;
	//K_d = 0.1; //3.16/10;
	FILE *fp2 = fopen("tuning_22.txt","a+");
...
//		printf("%f\n",X_ref);
//		X_ref = 0.0;
		X_ref = 0.001*sin(2*PI*10*(i*0.00002));
//		X_ref = X_ref + (dt * ramp_rate);
...
////////////////////////////////////////End F=3 /////////////////////
	...
	MotorOutDAC(board);
	X826( S826_CounterStateWrite(board, 4, 0)	); 				// Stop tracking encoder position.
